...
impl AmiToken 
{
    ...
    pub fn burn(env: Env, address_from_to_burn: Address, amount_to_burn: u32) -> u32
    {
        let contract_admin: Address = env.storage().instance().get(&ADMIN).unwrap();
        contract_admin.require_auth();
        ...
        env.storage()
            .persistent()
            .get(&DataKey::BalanceMap(address_from_to_burn.clone()))
            .unwrap_or(0);

        user_balance -= amount_to_burn;

        env.storage().persistent().set(&DataKey::BalanceMap(address_from_to_burn.clone()), &user_balance);

        let mut supply: u32 = env.storage().instance().get(&SUPPLY).unwrap_or(0);
        supply -= amount_to_burn;
        env.storage().instance().set(&SUPPLY, &supply);
